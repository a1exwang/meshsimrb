...
options = { rate: 0.05, verbose: MeshSim::V_NORMAL, safe_on: true }
parser = OptionParser.new do |opt|
  ...
  opt.on('-p', '--prof', 'start profiling') do
    options[:prof] = true
  end
  opt.on('-S', '--safe-off', 'disable safe check and this improves performance') do
    options[:safe_on] = false
  end
end
parser.parse!
...
MeshSim.meshsim(options[:infile], options[:outfile], options[:rate], options[:verbose], options[:prof], options[:safe_on])